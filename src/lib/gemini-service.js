...
class GeminiService {
    constructor() {
        Object.defineProperty(this, "geminiApiKey", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "openRouterApiKey", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
     ...
     */
    initialize(geminiApiKey, openRouterApiKey, model) {
     ...
     */
    async generateContent(prompt, systemPrompt, files, model, rawFiles, enableThinking, abortSignal) {
        this.ensureInitialized();
        try {
            ...
            const contents = []; // Add files if provided
            ...
            if (this.currentModel.supportsImageGeneration) {
                ...
                const imageCandidates = response.candidates;
                ...
                return response.text || 'I attempted to generate an image, but no image data was returned in the response.';
            }
            else {
     ...
     */
    async generateContentWithHistory(prompt, systemPrompt, conversationHistory, files, model, rawFiles, enableThinking, abortSignal) {
        this.ensureInitialized();
        try {
            // Set the model if specified
            if (model) {
            ...
            for (const part of candidate.content.parts) {
                console.log('Processing part with history:', {
                    ...
                    hasInlineData: !!part.inlineData,
                    hasInline_data: !!part.inline_data,
                    hasImageData: !!part.imageData,
                    hasImage_data: !!part.image_data,
                    hasBlob: !!part.blob,
                ...
                if (part.text) {
                    ...
                    if (part.inline_data && part.inline_data.data) {
                    ...
                    else if (part.imageData && part.imageData.data) {
                        // Handle imageData format
                        console.log('Found imageData part:', {
                            mimeType: part.imageData.mimeType,
                            dataLength: part.imageData.data?.length || 0
                        });
                        const base64Image = part.imageData.data;
                        const mimeType = part.imageData.mimeType || 'image/png';
                        if (base64Image) {
                            imageData.push(`data:${mimeType};base64,${base64Image}`);
                        }
                    }
                    else if (part.image_data && part.image_data.data) {
                        // Handle image_data format
                        console.log('Found image_data part:', {
                            mimeType: part.image_data.mimeType,
                            dataLength: part.image_data.data?.length || 0
                        });
                        const base64Image = part.image_data.data;
                        const mimeType = part.image_data.mimeType || 'image/png';
                        if (base64Image) {
                            imageData.push(`data:${mimeType};base64,${base64Image}`);
                        }
                    }
                        ...
                        console.log('Found blob part:', {
                            ...
                            dataLength: part.blob.data?.length || 0
                        });
                        const base64Image = part.blob.data;
                        const mimeType = part.blob.mimeType || 'image/png';
                        if (base64Image) {
                            imageData.push(`data:${mimeType};base64,${base64Image}`);
                        }
                    }
     ...
     */
    async generateContentOpenRouter(prompt, systemPrompt, model, conversationHistory, files, rawFiles, abortSignal) {
        if (!this.openRouterApiKey) {
        ...
        if (rawFiles && rawFiles.length > 0) {
            const fileDescriptions = rawFiles.map(file => `File: ${file.name} (${file.type}, ${file.size} bytes)`).join('\n');
     ...
     */
    formatResponseWithImages(textContent, imageData) {
        const imageHtml = imageData.map((data, index) => `<img src="${data}" alt="Generated image ${index + 1}" style="max-width: 100%; height: auto; margin: 10px 0;" />`).join('');
        return `${textContent}\n\n${imageHtml}`;
    }
    /**
     * Fetch image from URI
     */
    async fetchImageFromUri(uri) {
        try {
     ...
     */ processImageCandidates(candidates) {
     ...
     */
    processTextCandidates(candidates) {
        const candidate = candidates[0];
        if (!candidate.content || !candidate.content.parts) {
     ...
     */
    processThinkingResponse(response, enableThinking) {
        ...
        for (const part of candidate.content.parts) {
            ...
            console.log('Thought value:', part.thought);
